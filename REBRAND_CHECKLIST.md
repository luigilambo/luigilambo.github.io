...
| Boot screen CRT mobile | `mirror_root/textures/boot_screen_mobile.png` (360×260) | Disco + wordmark "SHADER", "Shader Development Studio / Version 1.02", "Copyright (c) Shader Sweden AB, 2026…" | **✅ FATTO (2026-06-09)** — ricreato con wordmark **BZN15**, "BZN15 s.r.l. - Website / Version 1.00", "Copyright (c) BZN15 s.r.l. - 2026. All Rights Reserved." (no refuso). Stesse dimensioni 360×260 (la checklist diceva 360×274, valore errato → corretto). Originale Shader recuperato da git in `boot_screen_mobile.png.bak_rebrand`. | ✅ |
...
| Cosa cambiare | Dove (file) | Valore attuale | Azione consigliata | Diff. |
|---|---|---|---|---|
| Texture schermo/badge del computer 3D | `mirror_root/models/computer.glb` — immagine embedded idx 2 `commodore-shader` (webp 1024x415), material `commodore-logo`, nodo `logo` | Disco + wordmark "SHADER", pannello boot "WELCOME TO FARMTIDEN", badge "SuperPET SP9000™" | **✅ FATTO (2026-06-09)** — ri-autorata la webp `commodore-shader` (SHADER → wordmark **BZN15**; pannello FARMTIDEN svuotato; badge SuperPET SP9000™ mantenuto) **e** sostituita la webp `background_compressed` (idx 3) con la variante **blu brand BZN15** (1254×1254). GLB ri-pacchettizzato preservando Draco + struttura texture; sorgenti modificabili in `texture_computer_3d/`. Originale in `computer.glb.bak_rebrand`. | ✅ |
...
| Simon (calling) | `mirror_root/textures/simon_calling.webp` | Foto reale (44 KB) | **✅ FATTO (2026-06-08)**: nuova foto team BZN15 (1254×1254, fondo grigio come l'originale, no-alpha). PNG→WebP. Originale in `.bak_rebrand`. | ✅ |
| Filip (footer) | `mirror_root/textures/filip_footer_5.webp` | Foto reale (101 KB) | **✅ FATTO (2026-06-08)**: nuovo ritaglio team BZN15 (1124×1399), PNG→WebP **con trasparenza**. Originale in `.bak_rebrand`. | ✅ |
| Jake (at computer) | `mirror_root/textures/jake_computer.webp` | Foto reale (46 KB) | **✅ FATTO (2026-06-08)**: nuovo ritaglio team BZN15 (1089×1444), PNG→WebP **con trasparenza**. Originale in `.bak_rebrand`. | ✅ |
...
| Decals logo "SHADER" sui telefoni 3D (3 istanze) | `mirror_root/models/phones.glb` — image idx 2 `shader-logo` (webp 1024x290), material `shader-logo`, nodi `phone-1/2/3-logo` | Disco + wordmark "SHADER" come decal su 3 telefoni | **✅ FATTO (2026-06-09)** — texture `shader-logo` ri-autorata (SHADER → wordmark **BZN15**, webp 1024×290) **e** sostituito anche il fondo `background2_d` (idx 1, webp 1024×1024) con la variante **glow blu brand BZN15**. GLB ri-pacchettizzato preservando Draco + EXT_texture_webp (12 bufferView, offset ricalcolati). Sorgenti in `texture_phones_3d/`. Originale in `phones.glb.bak_rebrand`. | ✅ |
| Decal logo "SHREDDER" stile-Shader | `mirror_root/models/shredder.glb` — image idx 1 `shredder-logo` (webp 1024x290), material `shader-logo`, nodo `cogs` | Disco + wordmark "SHREDDER" nello stile esatto del brand Shader | **✅ FATTO (2026-06-09)** — texture `shredder-logo` ri-autorata (SHREDDER → wordmark **BZN15**, webp 1024×290). GLB ri-pacchettizzato preservando Draco + EXT_texture_webp + KHR_materials_ior/specular (5 bufferView, offset ricalcolati). Sorgenti in `texture_shredder_3d/`. Originale in `shredder.glb.bak_rebrand`. | ✅ |
